...
aheadphase = ceil((ligthlength*pahead)/switchtime);     %time to keep green phase ahead
turnphase = ceil((ligthlength*(1-pahead)/2)/switchtime);    %time to keep green phase when turning
totalphase = 6 + 2*aheadphase + 4*turnphase;    %to reset the phase
count =0;   %counter
phase=0;    
    ...
    for a = 1:config_m
        for b = 1:config_n
            ...
            if ( config(a,b) == 1 )
                ...
                lightI_m = (a - 1) ;
                lightI_n = (b - 1) * 12;
                %calculate local off set of phase for different crossroads
                localphase = phase+(a+b-2)*traveltime;
                while (localphase > totalphase)         %reset localphase if necessary
